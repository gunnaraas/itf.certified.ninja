...
-	Working with Assembly code is not easy, you need to know the version specific to the processor’s platform, how memory segmentation works and how processor codes will respond in protected and unprotected memory environments. NOT EASY AT ALL.
...
### Query Languages
A query language is specifically designed to retrieve data from databases. The term query is synonymous with question and a query language is specialized to ask questions. Structured Query Language (SQL) and Lightweight Directory Access Protocol (LDAP) are the two most popular query languages 
